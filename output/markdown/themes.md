...
| 7.7% | `publishing` | 0.0173 | 0.0704 | under | `publish` 25.4% (348) `read` 13.3% (171) `write` 12.1% (719) `vocabulary` 9.4% (178) |
...
**20.6%** of the maximum divergence from the repository, against a null median of 15.4% over 999 resamples of the same size — an excess of +5.2 points, and 25 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.
...
| 6.5% | `biology` | 0.0383 | 0.0027 | **over** | `morphology` 25.9% (21) `owl` 21.1% (24) `person` 8.4% (24) `glob` 7.2% (4) |
| 6.2% | `publishing` | 0.0167 | 0.0704 | under | `publish` 25.4% (348) `read` 13.3% (171) `write` 12.1% (719) `vocabulary` 9.4% (178) |
| 5.1% | `politics` | 0.0552 | 0.0125 | **over** | `vote` 49.5% (53) `normalisation` 22.3% (23) `reading` 7.3% (691) `name` 5.4% (799) |
| 3.9% | `linguistics` | 0.3029 | 0.1972 | **over** | `word` 31.6% (1,535) `name` 15.3% (704) `term` 13.2% (378) `parse` 5.0% (156) |
...
| 2.8% | `literature` | 0.0129 | 0.0228 | under | `header` 37.7% (87) `write` 16.6% (718) `text` 12.5% (89) `prose` 8.1% (119) |
...
| 13.3% | `plants` | 0.0669 | 0.0304 | **over** | `root` 53.4% (162) `tree` 38.8% (200) `seed` 5.1% (49) `leaf` 1.9% (33) |
...
| 2.7% | `publishing` | 0.0918 | 0.0704 | **over** | `publish` 25.4% (348) `read` 13.3% (171) `write` 12.1% (719) `vocabulary` 9.4% (178) |
| 2.5% | `astrology` | 0.0159 | 0.0080 | **over** | `placement` 95.0% (107) `term` 2.0% (394) `figure` 1.3% (107) `descendant` 1.1% (7) |
...
| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 11.8% | `linguistics` | 0.0757 | 0.1972 | under | `word` 31.6% (1,535) `name` 15.3% (704) `term` 13.2% (378) `parse` 5.0% (156) |
...
| 12.5% | `electronics` | 0.1016 | 0.0161 | **over** | `tsv` 87.8% (42) `field` 5.8% (204) `headroom` 2.9% (2) `bar` 1.1% (106) |
| 7.4% | `linguistics` | 0.0901 | 0.1972 | under | `word` 31.6% (1,535) `name` 15.3% (704) `term` 13.2% (378) `parse` 5.0% (156) |
...
| 6.7% | `publishing` | 0.0175 | 0.0704 | under | `publish` 25.4% (348) `read` 13.3% (171) `write` 12.1% (719) `vocabulary` 9.4% (178) |
| 5.1% | `grammar` | 0.0797 | 0.0277 | **over** | `phrase` 21.1% (148) `occurrence` 15.7% (173) `part_of_speech` 13.9% (37) `verb` 13.5% (74) |
| 4.4% | `linguistics` | 0.3024 | 0.1972 | **over** | `word` 31.6% (1,535) `name` 15.3% (704) `term` 13.2% (378) `parse` 5.0% (156) |
...
**12.1%** of the maximum divergence from the repository, against a null median of 9.4% over 999 resamples of the same size — an excess of +2.7 points, and 36 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 12.1% | `plants` | 0.0001 | 0.0304 | under | `root` 53.4% (162) `tree` 38.8% (200) `seed` 5.1% (49) `leaf` 1.9% (33) |
| 6.5% | `electronics` | 0.0000 | 0.0161 | under | `tsv` 87.8% (42) `field` 5.8% (204) `headroom` 2.9% (2) `bar` 1.1% (106) |
...
| 3.7% | `linguistics` | 0.2735 | 0.1972 | **over** | `word` 31.6% (1,535) `name` 15.3% (704) `term` 13.2% (378) `parse` 5.0% (156) |
| 3.2% | `astrology` | 0.0000 | 0.0080 | under | `placement` 95.0% (107) `term` 2.0% (394) `figure` 1.3% (107) `descendant` 1.1% (7) |
| 2.6% | `administration` | 0.0001 | 0.0067 | under | `branch` 69.3% (143) `foreign` 10.7% (11) `title` 9.6% (12) `english` 4.0% (75) |

### `documentation`

**23.5%** of the maximum divergence from the repository, against a null median of 34.9% over 999 resamples of the same size — an excess of -11.5 points, and 980 of those draws diverged at least as far. **The scope does not stand outside its own null**, so it has no topical content beyond its size and nothing is ranked.

Read, compared and resampled in 10.7 s, recorded rather than estimated.
