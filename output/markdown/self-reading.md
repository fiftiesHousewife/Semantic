...
| `code-semantics-engine/src/main/java` | 184 | 3,964 | 5,646 | 39,517 (87.5%) | 44,487 | **0.985** |
| `code-semantics-engine/src/test/java` | 174 | 3,371 | 10,825 | 8,298 (43.4%) | 18,959 | **0.991** |
...
| **repository** | 550 | 10,263 | 24,263 | 72,867 (75.0%) | 95,449 | **0.983** |

The repository's own words are 97,130 occurrences of 3,984 distinct surfaces, 1,261 of them (31.7%) written exactly once. 231 glued runs no boundary divided were read by the segmenter; 1,681 occurrences across 472 surfaces nothing could be cited for at all.

| Resource | Occurrences it can be cited for | Share | Resting on it alone |
|---|--:|--:|--:|
| `lexicon:word-frequency` | 92,120 | 94.8% | 4,438 |
| `lexicon:wordnet-sense` | 66,269 | 68.2% | 1,174 |
| `lexicon:wiktionary-topic` | 54,235 | 55.8% | 217 |
| `lexicon:wikidata-initialism` | 43,477 | 44.8% | 77 |
| `lexicon:wikidata-name` | 38,807 | 40.0% | 44 |
| `lexicon:wordnet-domain` | 38,631 | 39.8% | 117 |
| `lexicon:wiktionary-abbreviation` | 33,465 | 34.5% | 10 |
| `catalogue:query-language-function` | 1,454 | 1.5% | 1 |
...
| `file's` | 21 | `code-semantics-api/src/main/java/io/github/fiftieshousewife/codesemantics/name/WordRanks.java:14` |
| `net's` | 21 | `code-semantics-api/src/main/java/io/github/fiftieshousewife/codesemantics/model/EvidenceSource.java:23` |

Read 550 files in 1.6 s — 3.0 ms a file, recorded rather than estimated.

Imports read as this repository's own choice: 83. Set aside as the platform's own vocabulary: 1525. Set aside as this repository's own coordinates: 519. Set aside as the toolchain a source set nothing is published from is checked with: 888. Files the parser refused: 0.