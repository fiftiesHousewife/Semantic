...
|---|--:|--:|--:|--:|--:|--:|---|
| `linguistics` | 0.1992 | 80.3% | 2,989 | 109 | 8,641 | 33.8% | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| `computing` | 0.0534 | 82.7% | 4,265 | 22 | 1,564 | 6.1% | `repository`&nbsp;347 `unread`&nbsp;9 `read`&nbsp;582 `parse`&nbsp;102 |

Read over 360 files and 25,534 lines: 466 topics, 1 files no topic could be resolved for, and 33.8% of the repository's lines led by its single strongest topic. ι is a share of topical mass and sums to 1 across every topic; the columns beside it are counts and do not. **From names** is how much of a theme the code itself carried rather than its prose.

## What distinguishes each scope from the repository

### `code-semantics-api/src/main/java`

**0.1554 bits** from the repository, against a null median of 0.0974 over 999 resamples of the same size — an excess of +0.0580, and 1 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 14.0% | `telecommunication` | 0.0002 | 0.0451 | under | `file`&nbsp;173 `entry`&nbsp;147 `note`&nbsp;9 `record`&nbsp;3 |
| 6.6% | `plants` | 0.0019 | 0.0289 | under | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
| 6.5% | `physics` | 0.0703 | 0.0210 | **over** | `mass`&nbsp;116 `weight`&nbsp;49 `radius`&nbsp;4 `length`&nbsp;35 |
| 5.5% | `publishing` | 0.0142 | 0.0529 | under | `publish`&nbsp;181 `read`&nbsp;118 `directory`&nbsp;51 `write`&nbsp;355 |
...
| 2.8% | `lexicography` | 0.0367 | 0.0128 | **over** | `citation`&nbsp;93 `sense`&nbsp;304 `gloss`&nbsp;2 `lemma`&nbsp;70 |

### `code-semantics-api/src/test/java`

**0.1966 bits** from the repository, against a null median of 0.1546 over 999 resamples of the same size — an excess of +0.0419, and 56 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 9.3% | `telecommunication` | 0.0017 | 0.0451 | under | `file`&nbsp;173 `entry`&nbsp;147 `note`&nbsp;9 `record`&nbsp;3 |
| 6.7% | `plants` | 0.0004 | 0.0289 | under | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
| 6.5% | `biology` | 0.0384 | 0.0031 | **over** | `morphology`&nbsp;16 `owl`&nbsp;31 `person`&nbsp;20 `glob`&nbsp;5 |
| 6.2% | `literature` | 0.0002 | 0.0261 | under | `header`&nbsp;68 `text`&nbsp;44 `write`&nbsp;355 `title`&nbsp;13 |
| 4.5% | `electronics` | 0.0005 | 0.0205 | under | `tsv`&nbsp;38 `field`&nbsp;150 `default`&nbsp;24 `node`&nbsp;51 |
| 4.2% | `politics` | 0.0553 | 0.0162 | **over** | `vote`&nbsp;53 `normalisation`&nbsp;13 `reading`&nbsp;406 `name`&nbsp;505 |
| 4.0% | `law` | 0.0971 | 0.0429 | **over** | `file`&nbsp;342 `witness`&nbsp;70 `summary`&nbsp;33 `within`&nbsp;8 |
| 3.5% | `publishing` | 0.0172 | 0.0529 | under | `publish`&nbsp;181 `read`&nbsp;118 `directory`&nbsp;51 `write`&nbsp;355 |

### `code-semantics-engine/src/main/java`

**0.0605 bits** from the repository, against a null median of 0.0232 over 999 resamples of the same size — an excess of +0.0373, and 0 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.
...
| 5.6% | `grammar` | 0.0508 | 0.0245 | **over** | `phrase`&nbsp;136 `verb`&nbsp;66 `occurrence`&nbsp;113 `clause`&nbsp;24 |
| 3.2% | `linguistics` | 0.2480 | 0.1992 | **over** | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| 2.5% | `politics` | 0.0300 | 0.0162 | **over** | `vote`&nbsp;53 `normalisation`&nbsp;13 `reading`&nbsp;406 `name`&nbsp;505 |
...
| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 16.1% | `plants` | 0.0816 | 0.0289 | **over** | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
...
| 2.8% | `christianity` | 0.0003 | 0.0045 | under | `kind`&nbsp;47 `use`&nbsp;67 `publish`&nbsp;181 `publisher`&nbsp;58 |
...
| 2.3% | `astrology` | 0.0142 | 0.0057 | **over** | `placement`&nbsp;44 `figure`&nbsp;62 `term`&nbsp;256 `configuration`&nbsp;3 |

### `lexicon-extraction/src/main/java`

**0.1690 bits** from the repository, against a null median of 0.0566 over 999 resamples of the same size — an excess of +0.1123, and 0 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 8.7% | `linguistics` | 0.0914 | 0.1992 | under | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| 7.0% | `literature` | 0.0847 | 0.0261 | **over** | `header`&nbsp;68 `text`&nbsp;44 `write`&nbsp;355 `title`&nbsp;13 |
| 6.1% | `plants` | 0.0018 | 0.0289 | under | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
...
| 4.5% | `physics` | 0.0013 | 0.0210 | under | `mass`&nbsp;116 `weight`&nbsp;49 `radius`&nbsp;4 `length`&nbsp;35 |
| 4.1% | `electronics` | 0.0587 | 0.0205 | **over** | `tsv`&nbsp;38 `field`&nbsp;150 `default`&nbsp;24 `node`&nbsp;51 |

### `lexicon-extraction/src/test/java`

**0.1984 bits** from the repository, against a null median of 0.0720 over 999 resamples of the same size — an excess of +0.1264, and 0 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 19.3% | `chemistry` | 0.1461 | 0.0206 | **over** | `syn`&nbsp;15 `extraction`&nbsp;48 `extract`&nbsp;7 `test`&nbsp;169 |
| 15.8% | `electronics` | 0.1281 | 0.0205 | **over** | `tsv`&nbsp;38 `field`&nbsp;150 `default`&nbsp;24 `node`&nbsp;51 |
| 6.1% | `linguistics` | 0.1002 | 0.1992 | under | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| 3.7% | `physics` | 0.0015 | 0.0210 | under | `mass`&nbsp;116 `weight`&nbsp;49 `radius`&nbsp;4 `length`&nbsp;35 |
| 3.2% | `person` | 0.0028 | 0.0227 | under | `reader`&nbsp;100 `child`&nbsp;18 `host`&nbsp;27 `bearer`&nbsp;18 |
| 3.0% | `lexicography` | 0.0002 | 0.0128 | under | `citation`&nbsp;93 `sense`&nbsp;304 `gloss`&nbsp;2 `lemma`&nbsp;70 |
| 2.6% | `law` | 0.0146 | 0.0429 | under | `file`&nbsp;342 `witness`&nbsp;70 `summary`&nbsp;33 `within`&nbsp;8 |
| 2.1% | `politics` | 0.0023 | 0.0162 | under | `vote`&nbsp;53 `normalisation`&nbsp;13 `reading`&nbsp;406 `name`&nbsp;505 |
...
| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 8.9% | `plants` | 0.0000 | 0.0289 | under | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
...
| 5.4% | `physics` | 0.0006 | 0.0210 | under | `mass`&nbsp;116 `weight`&nbsp;49 `radius`&nbsp;4 `length`&nbsp;35 |
| 4.3% | `networking` | 0.0338 | 0.0070 | **over** | `resource`&nbsp;194 `request`&nbsp;19 `preamble`&nbsp;5 `endpoint`&nbsp;12 |
| 3.8% | `linguistics` | 0.2910 | 0.1992 | **over** | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| 3.5% | `publishing` | 0.0196 | 0.0529 | under | `publish`&nbsp;181 `read`&nbsp;118 `directory`&nbsp;51 `write`&nbsp;355 |
| 3.1% | `law` | 0.0150 | 0.0429 | under | `file`&nbsp;342 `witness`&nbsp;70 `summary`&nbsp;33 `within`&nbsp;8 |

### `lexicon/src/test/java`

**0.1519 bits** from the repository, against a null median of 0.1176 over 999 resamples of the same size — an excess of +0.0342, and 41 of those draws diverged at least as far. The scope stands outside its own null, so its topics are ranked below.

| Share of the divergence | Topic | In scope | In repository | | Carried by |
|--:|---|--:|--:|---|---|
| 9.1% | `linguistics` | 0.3427 | 0.1992 | **over** | `word`&nbsp;1,072 `name`&nbsp;445 `term`&nbsp;248 `parse`&nbsp;102 |
| 9.0% | `plants` | 0.0002 | 0.0289 | under | `root`&nbsp;99 `tree`&nbsp;112 `seed`&nbsp;20 `leaf`&nbsp;11 |
| 6.7% | `electronics` | 0.0000 | 0.0205 | under | `tsv`&nbsp;38 `field`&nbsp;150 `default`&nbsp;24 `node`&nbsp;51 |
| 5.4% | `physics` | 0.0010 | 0.0210 | under | `mass`&nbsp;116 `weight`&nbsp;49 `radius`&nbsp;4 `length`&nbsp;35 |
| 5.1% | `lexicography` | 0.0477 | 0.0128 | **over** | `citation`&nbsp;93 `sense`&nbsp;304 `gloss`&nbsp;2 `lemma`&nbsp;70 |
| 3.5% | `politics` | 0.0014 | 0.0162 | under | `vote`&nbsp;53 `normalisation`&nbsp;13 `reading`&nbsp;406 `name`&nbsp;505 |
| 2.4% | `baseball` | 0.0208 | 0.0052 | **over** | `field`&nbsp;150 `outer`&nbsp;6 `line`&nbsp;176 `look`&nbsp;32 |
| 2.1% | `military` | 0.0031 | 0.0154 | under | `repository`&nbsp;347 `column`&nbsp;42 `base`&nbsp;46 `refuse`&nbsp;100 |

### `documentation`

**0.2448 bits** from the repository, against a null median of 0.3486 over 999 resamples of the same size — an excess of -0.1038, and 981 of those draws diverged at least as far. **The scope does not stand outside its own null**, so it has no topical content beyond its size and nothing is ranked.
