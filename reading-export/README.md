...
- The probes — `wordVotes`, `topicCarriers`, `pathPlacement`, `evaluationAnswers` and the rest — each prints one question's answer for a human; none of them writes under `output/`. `./gradlew tasks --group verification` lists them.
