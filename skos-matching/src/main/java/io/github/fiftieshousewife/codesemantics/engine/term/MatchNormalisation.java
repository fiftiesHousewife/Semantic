 ...
 *
 * <p>A term index answers on one normalisation and says which, because a match found on the words themselves and a
 * match found on what the dictionary says those words mean are not the same evidence and pooling them into one
 * figure would hide the whole question. The first normalisation cites nothing — it is a string comparison, and its
 ...
 *
 * <p>The middle normalisation is there because without it the last one takes the credit for its work. A taxonomy
 ...
 *
 * <p>The order is the order they are tried in: the narrowest normalisation that answers is the one that answers, and a
 * broader one is asked only where the narrower said nothing.
 */
public enum MatchNormalisation {
    ...
    MatchNormalisation(final String normalisation) {
