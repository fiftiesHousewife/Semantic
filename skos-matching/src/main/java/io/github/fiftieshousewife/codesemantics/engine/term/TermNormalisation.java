 ...
 *
 * <p>A normalisation is nothing more than this plus the index it is applied to, which is why there is one
 * decorator and not one per normalisation. The published terms go through it when the index is built and a repository's
 ...
 *
 * @param <K> what a run of words becomes — the type is the normalisation's own, and two normalisations never share a
 * key space
 */
public interface TermNormalisation<K> {
    ...
    /** The normalisation a match made through this normalisation is recorded at. */
