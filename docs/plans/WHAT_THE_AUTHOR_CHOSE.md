...
- **The cited catalogues.** `CitedTokens.NONE` recognises nothing, so `userid` refuses to split — `id` ranks 4690 and a two-letter piece must rank inside 1000. The Wikidata initialism registry looks like it would fill the seam and does not: `CitedTokenCatalogueTest` measures 14,322 three-letter tokens and 1,195 of the ten thousand commonest English words, including `CODE`, `DATA`, `NAME`, `TYPE`, `LIST`, `NODE`, `SIZE`. Narrowing it by the prominence the registry itself states is a **bound, so it must be derived** — on repositories this reading was not written for. Two computing-specific catalogues exist whose publisher is a standards body stating its own usage, which is the class the doctrine prefers over any registry of everything (surveyed 2026-08-19): the [RFC Production Center's abbreviations list](https://rpc-wiki.rfc-editor.org/rpc/wiki/doku.php?id=abbrev_list) — thousands of rows of abbreviation, expansion and a well-known marker, kept by the body that edits every RFC, with no licence stated on the page, so the licence question is the whole of the pre-work — and the [NIST CSRC glossary](https://csrc.nist.gov/glossary), a JSON export regenerated daily whose entries carry acronyms with expansions, a U.S. government work whose individual definitions may cite laws carrying their own terms. A third route bundles nothing new: the Wiktionary resource already shipped states its own topic categories, and its computing initialisms are a restriction the publisher itself states, the same move as the prominence narrowing above. The fixture-name reading supplies a live case for any of the three: this tree's own `.tsv` suffix reaches `electronics`, because the resources know `tsv` as the through-silicon via and nothing states the tab-separated values a computing catalogue would.
