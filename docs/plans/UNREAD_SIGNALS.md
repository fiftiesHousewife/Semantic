...
**Landed.** IANA's [licensing terms](https://www.iana.org/help/licensing-terms) place the protocol registries under the CC0 1.0 dedication, and the kill test did not kill: of 849 media-type-shaped literals in Tika's Java sources, 828 carry words no constant name writes. `media-types.tsv` bundles the registry's ten per-registry CSVs — 2,344 identifiers byte for byte, the file set pinned by `ContentDigest` — and `WrittenMediaTypes` counts the literals byte-identical to one, beside the reading and summed into no vote; every other literal still contributes nothing, and no English is read out of any literal. On Tika: **446 distinct registered identifiers across 1,850 occurrences** — `application/json` 128, `text/plain` 128, `application/pdf` 79 — none reachable from a declared name, since no name can spell a `/`. The ~400 media-type-shaped literals that are not in the registry — `x-raw-epson`, private `vnd.` strings — are refused for want of a citation, which is the rule doing its work. This tree writes 6. The extraction follows the shape [the extractors plan](THE_EXTRACTORS.md) will consolidate; it reuses `ContentDigest` and adds no new machinery of its own.
