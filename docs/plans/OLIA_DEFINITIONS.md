...
**They are visible now, which is a different thing and was worth doing first.** The taxonomy page names all
1,311 concepts rather than the 56 this repository writes, each with the publisher's own definition, and the
1,255 it does not write sit behind one closed fold per branch — the same rule at every depth, including the
59 root branches of the field this codebase never enters. The picture cannot do this and no styling will make
it: a sunburst wedge is as wide as its share of the field, so a leaf is a hairline and its name will not fit
inside it. A reader asking what this library *should* be matching against was reading the resource file to
find out, and the answer belongs in the reading.
