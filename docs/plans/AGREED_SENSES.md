...
| The vocabulary path and the topical path disagree about single letters | `x` is the 10th most distinctive word in Strata and never reaches a topic | `x`, `i`, `j`, `t`, `k` are in `signals` and absent from `workings.words` |
