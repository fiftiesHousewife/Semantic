...
[`fetch/fetch-candidate-vocabularies.sh`](../.fetch/fetch-candidate-vocabularies.sh) fetches every candidate's source into `~/vocabulary-sources`, recording each file's pin and sha256 in a manifest. It needs a shell with network access — the agent sandbox and the extraction JVMs have none — and an extraction then takes the fetched copy by path.
