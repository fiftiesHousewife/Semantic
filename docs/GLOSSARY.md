...
| **Least common subsumer** | The nearest concept two others are both a kind of. Its **depth** is how near they really are — two concepts meeting only at the root have been shown to be unrelated | the unbuilt hypernym-depth path |
...
| **Unithood** | How strongly several adjacent words hold together as one lexical unit. Kageura and Umino's own definition is "the degree of strength or stability of syntagmatic combinations and collocations". Answered here by citation — a run is one unit where a publisher states it as one entry. [Kageura and Umino 1996](https://www.jbe-platform.com/content/journals/10.1075/term.3.2.03kag) | `PublishedPhrases`, `CollocatedWords` |
| **Termhood** | How far a unit belongs to a subject rather than to the language everyone writes — "the degree to which a stable lexical unit is related to some domain-specific concepts", in the same paper. Answered here against a corpus of working Java: a run that corpus writes is common software vocabulary whatever taxonomy states it | `WorkingJavaRuns`, `SpecificTerms` |
| **Run** | Several adjacent words some publisher states as one entry, read as one unit. `time zone` is a WordNet entry, `interest rate swap` a FIBO concept label. Which runs exist is the publishers' answer, never this library's | `PublishedPhrases`, `StatedRuns`, `PooledRunShares` |
