...
- **One bar for both nulls, and one page** — `ChanceExpectedBest` is the `1/(n+1)` quantile in one place,
  read from the near end where smaller is nearer and the far end where larger is further. `SubjectNull`
  already held a placement to the best of 152 chance attempts; `PermutationNull` held a scope to the *median*
  of its own draws, which is a coin flip, and every scope of a repository is tested so the scopes are a field
  competing to look furthest. On this tree the corrected bar withholds **three scopes of nine instead of
  one** — two had been qualifying at rates chance produced in more than a quarter of draws, and each was
  printing eight ranked topics. `summary.md` is built only from what cleared a bar: the denominator, the
  field placement, and the qualified scopes, with what failed named beside the bar it failed.
- **A finding that closes off a whole line of work.** The summary's topic labels were noise, so a bar was put
  on the topics themselves — a topic one word carries a majority of is that word's opinion, which is
  `TopicWitnesses.restsOnOneWord` and is derived rather than tuned. It replaced `dentistry`,
  `christianity` and `horse-racing` with `chemistry`, `military` and `baseball`. **A label cannot be repaired
...
Three. The second decides whether the term reading means anything; the third is what would let a
reader check it.
...
### 3. The taxonomy states more than we read — definitions, provenance, and its other modules
...
## [HIGH] What a repository depends on is a statement about its domain
