...
|---|---|--:|---|---|
| **NEXT** | [The reports](docs/plans/THE_REPORTS.md) | 9 | **One statement of style for every generated artefact, and five defects against it.** The unselected-theme breakdown by package is clutter and goes; justification and line wrapping differ page to page and a wide table overflows rather than wrapping; a non-breaking space renders as literal text inside a table; **the taxonomy sunburst can no longer be enlarged or clicked**, which is a regression; and the same figure is restated in three reports in three wordings. The style is terse and factual, structure over prose, no metaphor, no word in a sense only this codebase uses, a worked example beside anything specialist, and a link's text is the class or word it names rather than its URL | nothing |
