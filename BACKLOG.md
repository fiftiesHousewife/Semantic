...
**The next four, in the order they are to be taken.** 1 unblocks every new vocabulary; 2 is the artefacts a
reader sees; 3 moves every reported figure, so it lands before anything is measured against them; 4 is the
first language beyond Java. Everything below them is ranked, not scheduled.
...
| **2** | [The reports](docs/plans/THE_REPORTS.md) | 9 | **One statement of style for every generated artefact, the README included, and the defects against it.** The unselected-theme breakdown by package is clutter and goes; justification and line wrapping differ page to page and a wide table overflows rather than wrapping; **the taxonomy sunburst can no longer be enlarged or clicked**, which is a regression; every report exists as both markdown and a mechanical HTML twin, and one of the two goes; and the README duplicates the reports rather than linking to them, where it should carry a bulleted summary, then how to run it, then links. The style rules now sit in `CLAUDE.md`: terse, factual, active voice, structure over prose, no metaphor in the text **or in a heading**, no word in a sense only this codebase uses, a worked example beside anything specialist, and a link's text is the class or word it names. Two of the defects are closed already — the entity that rendered as literal text inside a table, and the one metaphor heading. **The charts carry three of their own**: the README's bar explains where the bar came from rather than what it shows, the theme bar stacks segments a reader cannot compare and labels them illegibly, and neither chart lets a reader reach the words that carried a theme | nothing |
| **3** | [What the author chose](docs/plans/WHAT_THE_AUTHOR_CHOSE.md) | 1–3, 9 | **Three plans folded into one, eight of nine items landed, and the panel found the last two.** One question — did somebody choose this word, or did the language, the platform, the tool, the licence or this library's own arithmetic put it there? — answered in the vocabulary reading, for every later step to consume. **Landed**: a run the dictionary carries whole is one word (31 of 49 split runs here, 91 of 325 on Tika, where `extractor` read as extract / or 244 times); a catch parameter is the type it stands for (52 of 52 here, 1,675 of 1,744 on Tika); the possessive is not a boundary (UAX #29 WB6/WB7, and λ fell 0.992 → 0.984 because it had been counting a false citation); a stated inflection outranks a derived one, so `wa` 268 and `ha` 174 are gone and `netball` left the reading; a published run is one row, so `part_of_speech` stands 44th where `part` fell to 382nd; and **the cutoff is derived** — 259 of 859 words clear 0.000194 bits, holding 86.5% of the divergence against 88.2% on Tika, where the row counts differ threefold. **What is left, each with its number**: a comment copied into every file is 45.1% of Tika's comment words; a type restated in its own declaration's name is 21% of every OLiA span there; `doe` 191 survives because `does` is a regular form nothing states; and a weight read off a frequency list alone is blind to how often this repository wrote the word | nothing |
