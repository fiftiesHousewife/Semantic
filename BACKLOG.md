...
| **NEXT** | [The backtest](docs/plans/THE_PANEL.md) | all | Naming the members. The machinery, the manifest, `panelRead` and now `panelFetch` have landed — a member is fetched at the commit the manifest pins by a `backtest`-tagged test, excluded from every ordinary run like `generate`, `diagnostic` and `pinned`, and Tika arrives in 33 seconds. The table of every vocabulary against every repository needs members before it has anything to put in a cell | naming the members |
