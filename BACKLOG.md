...
**The next five, in the order they are to be taken.** 1 is the artefact a consumer actually wants and the
library does not produce; 2 unblocks every new vocabulary; 3 is what a reader sees; 4 moves every reported
figure, so it lands before anything is measured against them; 5 is the first language beyond Java. Everything
below them is ranked, not scheduled.

| | Plan | Step | What is left | Blocked on |
|---|---|--:|---|---|
| **1** | [The export](docs/plans/THE_EXPORT.md) | 9 | **One JSON file per run, holding everything a consumer would act on, written by default rather than behind a tag.** The strong signals — each word and published phrase that clears its derived bar, with its score, its occurrences and the line it was first written on — the themes the reading reports with their distance from chance, and the result of every taxonomy evaluation with the branch outcome and the subject placement beside it. **Signals and not rejects**: what was set aside is a count in the header, so nothing is hidden and no consumer reads a list of refusals to find the answer. `output/themes.json` is not this — 1.1 MB of one viewer's graph, carrying neither the vocabulary ranking nor any taxonomy result. **Two sources, one file**: the clone, and the pull requests a filter selects, starting with author id, each signal stating which source it came from so a reviewer can ask what an author's work adds to the picture. It leads the README when it lands | nothing |
...
| HIGH | [The backtest](docs/plans/THE_PANEL.md) | all | Naming the members. The machinery, the manifest, `panelRead` and now `panelFetch` have landed — a member is fetched at the commit the manifest pins by a `backtest`-tagged test, excluded from every ordinary run like `generate`, `diagnostic` and `pinned`, and Tika arrives in 33 seconds. The table of every vocabulary against every repository needs members before it has anything to put in a cell | naming the members |
...
- **The platform declares method names, and the reference now reads them.** `PlatformVocabulary` asked
  `ModuleFinder.ofSystem()` for the platform's *type* names, so it had nothing to say about `get` and `set`,
  which echo no type. `ClassFileMethods` reads each exported class file's constant pool and its
  `method_info` name indices — nothing loaded, linked, verified or initialised — and the whole platform
  enumerates in **259 ms**: 96,631 public and protected method names beside the 14,266 type names. The
  enumeration had been abandoned once as too slow, having reflected over every exported class; reflection was
  what was slow, not the question.

  **On the panel member `get` fell from first of 4,582 declared words to last of 4,527**, and `set` from 21st
  to 4,507th: the platform writes `get` at 9.2359% of everything it declares against Tika's 2.8621%, so the
  reference carries it as a negative claim. **Nothing of the subject matter moved** — `metadata` stands 1st,
  `config` 2nd, `extract` 11th, `detect` 20th — which is the criterion the plan set for abandoning it. The
  one word a reader might argue about is `stream`, at 168th, which the platform declares at 0.2756%.

- **A declared name that is the initials of its own type is that type quoted.** `TypeInitials` splits the
  declared type by the same grammar the name is split by and compares the first letter of each word;
  `NameForm.ABBREVIATED_TYPE` records what it claims, so the word is still counted as written and only stops
  counting as a name. It is `CAUGHT`'s rule reaching every declaration that writes its type out, and it is a
  rule about the parse rather than about length — `String id` spells `s`, so `id` is untouched where any
  threshold that removes `sb` would remove it too.

  **4,108 declarations over 242 distinct names on Tika, against one here.** `tis` of a `TikaInputStream`
  1,080, `i` of an `int` 962, `m` of a `Metadata` 244, `is` of an `InputStream` 228, `sb` of a
  `StringBuilder` 155. In the published names `tis` fell from **7th on 498 occurrences to 147th on 39**, and
  `sb` is no longer written as a name at all. The accident the plan asked to be reported is **one declaration
  in 4,108** — `id` of an `ImageDeskew`. This tree could show none of it: its single claimed name is `i` of
  an `int`, and `./gradlew abbreviatedTypes` prints every pair on any clone.

- **A panel member is fetched by the suite rather than by hand.** `PinnedClone` runs the shallow fetch the
  panel plan measured — `git init`, `git fetch --depth 1` naming the commit, `git checkout FETCH_HEAD` — and
  does nothing where the tree is already at the pinned commit, so a second run costs nothing. It is a test
  tagged **`backtest`**, excluded from every ordinary run beside `generate`, `diagnostic` and `pinned`,
  because it reaches the network. `./gradlew panelFetch -Dcs.panel.dir=<directory>` is the whole of it, and
  Tika arrives at `43cbdae6` in 33 seconds.

- **The vocabulary reading no longer prints a number of rows somebody chose.** `VocabularyNull` draws a
  repository of this one's size from each reference's own distribution, scores every drawn word by the same
  term of the same divergence the ranking uses, and reads the quantile `ChanceExpectedBest` gives for a
  field of this many words. One bar per reference and a word clears each, which is the ranking's own
  weakest-claim rule applied to the bound. It passes both tests set for it in advance: every word a mass
  cutoff would have wrongly dropped clears it — `arxiv`, `fibo`, `csf`, `hypernym`, `olia`, `skos`, `nist`
  and `segmenter`, each written three to six times — and the share of the divergence the rows hold agrees
  to within two points across two repositories whose row counts differ by a factor of three.
...
- **A word English put inside a name is told from a word the author chose, and neither reference could do
  ...
  **The report's own cutoff was measured while this landed, and it is picked.** `./gradlew vocabularyMass`
  ...
  where every resource this library reads is named. [The plan](docs/plans/WHAT_THE_AUTHOR_CHOSE.md) states the
