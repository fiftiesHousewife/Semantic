...
|---|---|--:|---|---|
| **NEXT** | [What the author chose](docs/plans/WHAT_THE_AUTHOR_CHOSE.md) | 1–3, 9 | **Three plans folded into one, because they asked one question in three places: did somebody choose this word, or did the language, the platform, the tool, the licence or this library's own arithmetic put it there?** The answer belongs in the vocabulary reading, and every later step consumes it rather than re-deriving it. Nine items in a stated order — a cleanup that changes what the ranking contains must land before any bound that cuts it, and before the ranking is run on a repository it was not written for. **The two cheapest are measured and settled**: the segmenter split 49 distinct runs over 266 occurrences of this tree's declared names and **the dictionary carries 31 of them whole over 134 occurrences** — `abstains` as ab / stains 22 times, `synset` as syn / set, `enclosing` as enc / losing, `annotation` as an / notation, `splitter` as sp / litter — and **every catch parameter in the tree is `e`**, 52 of them, which is the whole of `e` standing thirteenth in the ranking. Then the possessive, the lemmatiser's `wa` 268 / `doe` 191 / `ha` 174, a published run as one row, a weight that is observed against expected rather than frequency alone, the permutation null that derives the cutoff, the run on another repository, and step 8 reading the partition. Two more cannot be seen on this tree at all: a comment copied into every file (**45.1% of Tika's comment word occurrences**) and a type restated in its own declaration's name (**21% of every OLiA span on Tika**) | nothing |
