...
|---|---|--:|---|---|
| **NEXT** | [The platform's own method names, and a name that is its own type](docs/plans/WHAT_THE_AUTHOR_CHOSE.md) | 1–2, 8 | **The two the panel exposed, both with a number and neither visible on this tree.** `get` stands **first** of Tika's 4,582 declared words at 2,652 occurrences, 2.80% of everything it declared, with `set` 21st at 1,343 — because `PlatformVocabulary` asks `ModuleFinder.ofSystem()` for the **type** names the platform exports and these are **method** names. This codebase declares no getters, so `get` never appears here as a declared name at all. The enumeration was abandoned once for the wrong reason, having reflected over every exported class; the method names are in the class file, and `ModuleReader.open(...)` plus the constant pool and the `method_info` name indices reach them without loading, verifying or initialising anything. **And a declared name that is its own type abbreviated**: `tis` 498 stands seventh on Tika and is a `TikaInputStream`, with `is` 146 an `InputStream`, `sb` 146 a `StringBuilder`, `ch` 118 a `char` and `cp` 80 a code point. That is `List<Foo> getFooList()` seen from the other end and wants the same parse rule generalised to the initials of the declared type's words — no list, and no rule about length, which is what it replaces, since any threshold that removes `sb` removes `id` with it | nothing |
| **NEXT** | [What the author chose](docs/plans/WHAT_THE_AUTHOR_CHOSE.md) | 1–3, 9 | **Three plans folded into one, six of nine items landed, and the panel found the seventh.** One question — did somebody choose this word, or did the language, the platform, the tool, the licence or this library's own arithmetic put it there? — answered in the vocabulary reading, for every later step to consume. **Landed**: a run the dictionary carries whole is one word (31 of 49 split runs here, 91 of 325 on Tika, where `extractor` read as extract / or 244 times); a catch parameter is the type it stands for (52 of 52 here, 1,675 of 1,744 on Tika); the possessive is not a boundary (UAX #29 WB6/WB7, and λ fell 0.992 → 0.984 because it had been counting a false citation); a stated inflection outranks a derived one, so `wa` 268 and `ha` 174 are gone and `netball` left the reading; a published run is one row, so `part_of_speech` stands 44th where `part` fell to 382nd; and **the cutoff is derived** — 259 of 859 words clear 0.000194 bits, holding 86.5% of the divergence against 88.2% on Tika, where the row counts differ threefold. **What is left, each with its number**: `get` stands **first** in Tika's ranking at 2,652 occurrences because `PlatformVocabulary` reads type names and these are method names; `tis` 498, `is`, `sb`, `cp` and `ch` are declarations abbreviating their own type; a comment copied into every file is 45.1% of Tika's comment words; a type restated in its own declaration's name is 21% of every OLiA span there; `doe` 191 survives because `does` is a regular form nothing states; and a weight read off a frequency list alone is blind to how often this repository wrote the word | nothing |
