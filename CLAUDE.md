...
- **Define an ambiguous term where it first appears, and link the definition somebody else published.** *Sense*, *scope*, *span*, *mass*, *normalisation* all carry an everyday meaning and a technical one. Give the technical one in a clause at first use and link the publisher who states it — for *sense*, WordNet's own documentation. Where no external definition exists, **use a commoner English word, or the standard term of the relevant specialist field — never a meaning invented here.** A term this project genuinely coined belongs in [`docs/GLOSSARY.md`](docs/GLOSSARY.md), defined, and linked at first use.
...
- **An artefact states; it does not say.** A file, a prefix, a vocabulary and a tracker all publish, and none of them speaks. *States*, *defines*, *records*, *lists* and *names* stay; *says*, *answers*, *speaks* and *tells* go. *Calls* survives where it means naming — *what Jira calls a Bug* is what Jira wrote in the field.
- **One possessive per noun phrase.** *The word a published standard's own definition covers the measured shape with* stacks three owners before the noun; write *the type whose definition covers what the parse measured*. The fix is a relative clause — *whose*, *that*, *the … of …*.
- **A pronoun's referent is named in its own sentence.** *It*, *its*, *they* and *their* reach back one sentence at most, and never to a table heading or a page title. Cover everything above the sentence; if the subject has vanished, name it.
- **Never end a sentence on a preposition.** *The word a standard's definition covers the measured shape with* leaves *with* five words from the noun it governs; write *the type whose definition covers what the parse measured*. Recasting usually wants a stronger verb, and a sentence closing on *with*, *on*, *from* or *for* is one whose verb was too weak to hold its object.
...
1. `./gradlew cleanTest checkAll --parallel` — **a cached `BUILD SUCCESSFUL` reads exactly like a real green.** Run `cleanTest` before trusting one. `--parallel` halves the wall clock (8m 38s against 18m 39s, measured 2026-09-12; the parallel gate itself is 3m 15s since the per-word memoisations of 2026-09-13) and is safe here because `dependencyUpdates`, the task `gradle.properties` disables parallel execution for, is not in `checkAll`.
