...
- A second pass in the spirit of one sense per discourse multiplies each sense's weight by 1 + the share the first pass gives its labels, bounded in [1, 2] by what a share is and exactly 1 for an unlabelled sense. `./gradlew discoursePass -Dcs.evaluation.dir=<clones>` prints both passes side by side. On the nine evaluation members it moves the top-three concentration by at most 1.3 points, changes no member's leading domain, and reorders one pair on one member, so every page draws the counts-alone pass.
