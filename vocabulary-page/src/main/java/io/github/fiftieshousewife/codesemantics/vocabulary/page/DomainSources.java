...
/**
 * What every bundled domain source made of one reading's words, as one row each, so the page states which
 * resources the domain claims rest on rather than drawing one picture and naming nobody.
 *
 ...
 * own words, and the rows are what a reader compares.
 *
 * <p>What can be compared without a mapping is coverage: how many of the same significant words a source
 * labelled with anything. The share of <em>weight</em> a source placed is not that comparison — a source
 * whose every sense carries a label by construction reads 100% however few words it reached — so the
 * rows count words, the same arithmetic for every source.
 */
record DomainSources(List<Row> rows, int significantWords) {
     ...
     *
     * @param source      the source's own name
    ...
    /**
     * Every source's row, the widest-covering source first.
     *
    ...
    static DomainSources of(final Map<String, DomainOverlap> bySource) {
        final List<Row> rows = bySource.entrySet().stream()
                ...
                .toList();
        return new DomainSources(rows, bySource.values().iterator().next().significantWords());
    }
