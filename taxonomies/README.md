# Taxonomies read by path rather than from the classpath

Each file here is a published taxonomy converted to the eight SKOS columns every bundled resource uses — `concept`, `prefLabel`, `altLabel`, `broader`, `kind`, `module`, `definition`, `note` — and carries the provenance header the bundled ones carry: what it was read from, at which revision, and under which licence.

**They are stored here and not in `lexicon/src/main/resources` on purpose.** A file under that directory is bundled into the published jar, and [the extractors plan](../docs/plans/THE_EXTRACTORS.md) states that no further vocabulary is bundled until the six repeated extraction steps are one runner. A taxonomy here is committed, diffable and reproducible without being published — and the directory says nothing about how much a run uses a file. Every run matches `cso-topics.tsv` and [the export](../README.md#the-export) carries its concepts beside the bundled OLiA's; `bian-service-domains.tsv` is read where a build points the functional placement at it.

Point a reading at one with `-Dcs.taxonomy=<path>`:

    ./gradlew functionalPlacement -Ptaxonomy=taxonomies/bian-service-domains.tsv

## What each file holds

| File | Publisher | Concepts | Read as | Licence |
|---|---|--:|---|---|
| `cso-topics.tsv` | [CSO](https://cso.kmi.open.ac.uk/), Knowledge Media Institute | 14,636 topics | a **term** taxonomy — CSO states no definition for any topic, so there is no prose to compare a scope against | CC BY 4.0, attribution required |
| `bian-service-domains.tsv` | [BIAN](https://github.com/bian-official/artefacts) | 319 service domains under 8 business areas | a **functional** taxonomy — each states a role definition | Apache-2.0, stated in the publisher's own `LICENSE` |

**What CSO finds.** Run against Apache Tika it corroborates 93 terms that describe a text-extraction toolkit — `encoding` 118, `detector` 181, `parse` 666, `xml` 354, `html` 264, `hyperlink` 47, `word processing`, and the metadata standards Tika reads in `gps` and `cad`. The bundled subject scheme places Tika and this repository under the same category, so CSO is the first source measured here that separates them. Two pronouns survive the citation rule — `it` and `its`, which WordNet carries as words and the registry states as *information technology*.

**`cso-topics.tsv` was lossy and no longer is.** CSO is a poly-hierarchy: 9,927 of its 14,624 topics with a parent have more than one, up to 33. Every parent is carried, joined with the separator `SkosConcept` states for a repeated property, and `broaderConcepts` reads them back — nothing is dropped. **Carrying them made the reading worse before it made it better**: branch corroboration asks whether a sibling was written, which discriminates in a tree and barely discriminates in a dense poly-hierarchy, and on Tika the rule fell from removing 90% of matches to removing 29%.

## What a file here still owes

The conversion that wrote each file is not in the tree, so nobody else can regenerate it. That is the debt: an extraction produces the same bytes from the pinned revision every time, and until one exists these files are artefacts somebody produced rather than readings anybody can repeat. The header names the revision, so the gap is visible. The files are tested where they are read — `TaxonomyIsKeyedByWordsTest` reads `cso-topics.tsv` on every build, and `ReachedSubjectTest` reads it with the diagnostics — so the extraction is the whole of what is owed.
